export default async function submitContactForm(data, context) {
  const { restaurant_name, contact_name, email, phone, service_type, urgency, message } = data;
  ...
  const response = await fetch(
    "https://api.airtable.com/v0/appL7jbLQnqVyg8kC/Base%20Stockage%20contact",
    {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${context.secrets.AIRTABLE_TOKEN}`,
